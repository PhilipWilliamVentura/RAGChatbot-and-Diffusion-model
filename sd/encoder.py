...
class VAE_Encoder(nn.Sequential):
    ...
    def __init__(self):
        super().__init__(
            # Look at https://ezyang.github.io/convolution-visualizer/index.html to visualize

            # 3 -> 128 features, size stays the same because of padding
    ...
    def forward(self, x: torch.Tensor, noise: torch.Tensor) -> torch.Tensor:
        ...
        for module in self:
            if getattr(module, 'stride', None) == (2, 2):
                # (Padding_Left, Padding_Right, Padding_Top, Padding_Bottom) -> apply to right and bottom
                x = F.pad(x, (0, 1, 0, 1))
            x = module(x)
